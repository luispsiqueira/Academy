...
class SeventhSpriteSceneCode: SKScene {
    ...
    override func didMove(to view: SKView) {
        ...
        attackbutton = childNode(withName: "Attack") as? SKSpriteNode
    ...
    override func update(_ currentTime: TimeInterval) {
        ...
        if (sophia?.position.x ?? 0 < 120) && (sophia?.position.y ?? 0 < 280) && (sophia?.position.x ?? 0 > -120) && (sophia?.position.y ?? 0 > 90){
            sophia?.position.y = -80
            sophia?.position.x = 5
            NotificationCenter.default.post(name: Notification.Name("Spider04"), object: nil)
        }
    }
}
