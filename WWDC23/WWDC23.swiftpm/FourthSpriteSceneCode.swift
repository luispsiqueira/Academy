...
class FourthSpriteSceneCode: SKScene {
    
    @State private var next02 = false
    ...
    override func update(_ currentTime: TimeInterval) {
        if sophia?.position.x ?? 0 < -140 && sophia?.position.y ?? 0 < -40{
            NotificationCenter.default.post(name: Notification.Name("Next View 2"), object: nil)
        }
        
        if (sophia?.position.x ?? 0 < -60) && (sophia?.position.y ?? 0 > 300){
